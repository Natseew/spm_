...
router.post('/accept-change', async (req, res) => {
    ...
    const id = await client.query(`
        SELECT staff_id from recurring_request WHERE requestid = $1
    `, [requestID]);
    
    const staff_id = id.rows[0].staff_id;

    // console.log("ALL WFH Records:",allWfhRecords)
    console.log("Staff ID:",staff_id);
    

    // Ensure changeDates are in date format (assuming changeDates is an array of date strings)
    const inOfficePercentages = await calculateInOfficePercentage(staff_id, changeDates, allWfhRecords.rows);
    ...
    for (const [date, timeslots] of Object.entries(inOfficePercentages)) {
        for (const [timeslot, percentage] of Object.entries(timeslots)) {
            ...
            if (percentage < 50) {
                console.log("Approval failed because this violates the more than 50% in office rule");
                await client.query('ROLLBACK');
                return res.status(200).json({
                    message: 'In-office percentage is below 50% for certain dates or timeslots',
                    date,
